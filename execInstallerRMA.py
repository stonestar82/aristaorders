...
if eq(platform.system().lower(), "windows"):
  path = "."
  icon = "icloud.ico"
  cmd = f'pyinstaller -F --icon=./{icon} --add-data="{icon};." --key={kkk} -w --distpath="release" -n=AristaRMA rma.py'
else:
  ## mac에서는 .ico는 인식안됨. -w 옵션이 들어가야 실행파일에 아이콘이 들어감
  path = os.path.sep.join(sys.argv[0].split(os.path.sep)[:-1])
  icon = "icloud.icns"
  cmd = f'pyinstaller -F --icon={icon} --add-data="{icon}:." --add-data="icloud.png:." --key={kkk} --distpath="release" -n=AristaRMA rma.py'
  
releaseFolder = f"{path}/release"
sampleXlsx = f"{path}/sample_rma.xlsx"
toSampleXlsx = f"{releaseFolder}/sample_rma.xlsx"
